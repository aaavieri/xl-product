...
export default {
  ...
  computed: {
    ...mapState('tab', { medicalData: state => state.productData.filter(item => item.tableName === 't_medical') })
  },
  methods: {
    cancelSearch () {
      this.searchText = ''
    ...
    generateTreeData (data) {
      ...
      data.map(item => {
        ...
        if (!typeData) {
          typeData = {
            id: item.typeId,
            name: medical.$dictionary.getName('t_medical', 'typeId', item.typeId),
            open: false,
            children: []
          }
  ...
  mounted () {
    this.generateTreeData(this.medicalData)
