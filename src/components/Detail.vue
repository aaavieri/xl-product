<template>
  <transition
    ...
    <md-card class="page-container no-border" v-show="startPageAnimate">
      ...
      <md-button class="md-icon-button md-accent md-raised md-des my-link my-link-down md-elevation-20"
                 @click="goBottom()">
    ...
    methods: {
      ...
      getOriginalData() {
        ...
        Object.keys(data).map((key) => {
          let columnInfo = detailPage.$columnInfo.getColumn(tableName, key)
      ...
      goComponent(selector) {
        ...
        let component = this.$el.querySelector(selector)
      ...
      generateCardColor() {
        let len = cardColorArr.length
        for (let i = 0; i < len - 1; i++) {
          let index = parseInt(Math.random() * (len - i))
          let temp = cardColorArr[index]
