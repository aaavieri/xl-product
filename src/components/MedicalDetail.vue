      ...
      <md-app md-waterfall md-mode="fixed" class="shape-app">
        ...
        <md-app-drawer class="my-drawer" md-permanent="clipped" :md-active.sync="menuVisible">
          ...
          <md-list class="my-drawer-list">
            <md-list-item class="my-drawer-item" v-for="info in medicalData"
...
export default {
  ...
  methods: {
    initMedicalDetail () {
      var originalMedicalData = this.getOriginalMedicalData()
      this.medicalData = this.structureMap.sort((i1, i2) => i1.index - i2.index).map(info => {
        info.fieldArray.sort((f1, f2) => f1.fieldIndex - f2.fieldIndex).map(field => {
          // field.fieldValue = originalMedicalData[field.fieldName]
          this.$set(field, 'fieldValue', originalMedicalData[field.fieldName])
          return field
        })
        return info
      })
    },
    getOriginalMedicalData () {
      let medicalDetail = this
      var medicalData = Object.assign({}, this.rowData)
      Object.keys(medicalData).map(key => {
        var columnInfo = medicalDetail.$columnInfo.getColumn('t_medical', key)
        if (columnInfo && columnInfo.dataType === 'dictionary') {
          medicalData[key] = medicalDetail.$dictionary.getName('t_medical', key, medicalData[key])
        }
      })
      if (medicalData.testCycleMin !== medicalData.testCycleMax) {
        medicalData.testCycle = `${medicalData.testCycleMin}-${medicalData.testCycleMax}`
      } else {
        medicalData.testCycle = `${medicalData.testCycleMin}`
      }
      return medicalData
      // return {
      //   serial: 'FE001',
  ...
  mounted () {
    this.initMedicalDetail()
    this.generateCardColor()
    this.startAnimate = false
    this.startPageAnimate = true
    // var medicalDetail = this
    // setTimeout(() => {
    //   medicalDetail.startAnimate = true
    // }, 10)
  }
}
</script>
